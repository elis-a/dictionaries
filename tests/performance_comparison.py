...
# Aggiungo un limite di ricorsione più alto per evitare crash dell'ABR con dati ordinati
sys.setrecursionlimit(6000)

project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
sys.path.insert(0, project_root)

# Tentativo di importazione delle strutture dati
try:
...
def run_performance_test(dict_class, keys_to_insert, keys_to_search, keys_to_delete):
    ...
    if dict_class == HashTableDict:
        # Trovo il primo numero primo successivo a len * 2 per ottimizzare la distribuzione.
        size = len(keys_to_insert) * 2 + 1
        while not all(size % i for i in range(2, int(size ** 0.5) + 1)):
            size += 2
    ...
    start_time_search_hit = time.perf_counter()  # misuro il tempo per la ricerca delle chiavi esistenti
    ...
    start_time_search_miss = time.perf_counter()  # misuro il tempo per la ricerca delle chiavi non esistenti
...
def plot_results(results_data, n, scenario):
    # Questa è la nuova funzione per creare e salvare un grafico a barre con i risultati.
    ...
    # Trovo il nome ABR nel caso ci siano variazioni nel testo come "ABR (Albero...)"
    abr_name = next(key for key in results_data if 'ABR' in key)
    abr_times = [results_data[abr_name][op] for op in ['insert', 'search_hit', 'search_miss', 'delete']]
    ...
    width = 0.20  # la larghezza delle barre

    fig, ax = plt.subplots(figsize=(15, 8))
    rects1 = ax.bar(x - width, list_times, width, label='Lista Concatenata', color='#64C5EB')
    rects2 = ax.bar(x, abr_times, width, label=abr_name, color='#FEB326')
    rects3 = ax.bar(x + width, hash_times, width, label='Tabella Hash', color='#E84D8A')
    ...
    # Questo è particolarmente utile quando ci sono ordini di grandezza diversi tra le strutture.
    ax.set_yscale('log')
...
def main():
    # Definisco il numero di elementi da testare
    sizes = [1000, 5000, 10000]

    # Le strutture dati da testare
    structures = {
    ...
    for n in sizes:
        ...
        results_random = {}  # Dizionario per raccogliere i risultati di questo scenario per il plotting
        for name, dict_class in structures.items():
            ...
            for op_name, op_time in results.items():
                all_results_for_csv.append({
                    ...
                    'Scenario': "Casuale",
                    'Structure': name,
                    'Operation': op_name.replace('_', ' ').title(),  # Formatto il nome dell'operazione per leggibilità
